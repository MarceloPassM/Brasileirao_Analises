...
with st.sidebar:
    ...
    rodada = st.slider("Até a rodada:", 1, max_rodada, max_rodada)

    st.markdown("---")
    st.markdown("""
**Páginas**
- 🏆 Classificação *(você está aqui)*
- 📈 Evolução na Tabela
- ⚔️ Head-to-Head
- 🥅 Análise de Gols
- 🔴 Zona de Rebaixamento
    """)
    st.markdown("---")
    st.caption("Dados simulados · Série A 2021–2025")
...
for col, label, val, sub in zip(
    [c1, c2, c3, c4],
    ["Líder", "Mais Gols Marcados", "Melhor Aproveitamento", "Menos Gols Sofridos"],
...
tabela_display.insert(0, "", [colorir_posicao(i) for i in range(1, len(tabela) + 1)])
...
🟢 Libertadores &nbsp;|&nbsp; 🔵 Sul-Americana &nbsp;|&nbsp; ⚪ Zona neutra &nbsp;|&nbsp; 🔴 Rebaixamento
</div>
""", unsafe_allow_html=True)
